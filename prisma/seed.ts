...
async function main() {
  ...
  // menu items - createMany bulk
  const menuItemsData = Array.from({ length: 80 }, (_, i) => {
    const cat = cats[i % cats.length];
    const itemName = menuNames[i % menuNames.length];
    return {
      restaurantId: rest.id,
      categoryId: cat.id,
      name: itemName + (i >= menuNames.length ? ` ${Math.floor(i / menuNames.length) + 1}` : ""),
      description: `Delicious ${itemName} with authentic spices`,
      price: 120 + (i * 13) % 350,
      isVeg: i % 3 !== 0,
      isAvailable: true,
    };
