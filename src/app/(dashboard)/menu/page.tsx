...
export default function MenuPage() {
  ...
  async function createCategory() {
    ...
    if(res.ok){ setCatName(""); setShowCatForm(false); setMsg("✓ Category created"); load(); } else { const j=await res.json(); setMsg(j.error||"Failed"); }
  }
  ...
  async function saveItem() {
    ...
    if(editingItem){
      ...
      if(res.ok){ const updated = await res.json(); setItems(prev=> prev.map(p=> p.id===editingItem.id? {...p, ...updated}:p)); setShowItemForm(false); setEditingItem(null); setMsg(`✓ ${payload.name} updated`); }
      ...
      if(res.ok){ setShowItemForm(false); setMsg(`✓ ${payload.name} created`); load(); } else { const j=await res.json(); setMsg(j.error||"Failed"); }
    }
  }
  ...
  async function deleteItem(item: MenuItem){
    ...
    setBusyId(null);
    if(res.ok){ setItems(prev=> prev.filter(p=> p.id!==item.id)); setMsg(`✓ ${item.name} deleted`); }
  ...
  return (
    <div className="space-y-6">
      ...
      {msg && <div className="text-sm bg-amber-50 border border-amber-200 rounded-lg p-3 flex justify-between items-center"><span>{msg}</span><button onClick={()=>setMsg("")} className="text-zinc-500 hover:text-zinc-700">✕</button></div>}

      {showCatForm && <Card className="border-orange-200"><CardHeader><CardTitle className="text-base">New Category</CardTitle><CardDescription>Slug auto-generated — e.g. Biryani & Rice → biryani-rice</CardDescription></CardHeader><CardContent className="flex gap-2"><Input placeholder="Category name e.g. Desserts" value={catName} onChange={e=>setCatName(e.target.value)} className="flex-1" /><Button onClick={createCategory} className="bg-zinc-900">Create</Button><Button variant="ghost" onClick={()=>setShowCatForm(false)}>Cancel</Button></CardContent></Card>}
        ...
        <Card className="border-zinc-200 shadow-lg">
          ...
          <CardContent className="grid gap-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2">
              ...
              <p className="text-xs text-zinc-500 mt-1">Use Unsplash/CDN link — 600×400 recommended. Leave empty for fallback.</p>
            </div>
            ...
            <Card key={m.id} className={`group overflow-hidden hover:shadow-lg transition-all border-zinc-200 ${!m.isAvailable ? "opacity-60 border-dashed bg-zinc-50" : "bg-white"} `}>
              <div className="relative">
                ...
                <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={()=> openEdit(m)} className="h-7 w-7 rounded-full bg-white shadow border flex items-center justify-center hover:bg-zinc-50 text-xs" title="Edit">✎</button>
