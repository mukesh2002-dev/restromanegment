...
export default function POSPage() {
  ...
  async function searchCustomer(){
    ...
    if(!/^[6-9]\d{9}$/.test(phone)){ setCustomerMsg("Invalid mobile â€” must be 10 digits starting 6-9"); return; }
    ...
    try{
      ...
      if(j.found && j.customer){
        ...
        setCustomerMsg(`Found: ${j.customer.name} â€¢ Visit ${j.customer.totalVisits||j.customer.totalOrders||0} â€¢ Spend â‚¹${Math.round(j.customer.totalSpending||j.customer.totalSpend||0)} â€¢ Points ${j.customer.loyaltyPoints||j.customer.loyaltyAccount?.points||0}`);
        ...
        setCustomerMsg("Customer not found â€” fill name to create new customer (Â§5)");
      }
    } catch{ setCustomerMsg("Search failed â€” try again"); }
  ...
  async function createNewCustomer(){
    ...
    try{
      ...
      const j = await r.json();
      if(!r.ok){ setCustomerMsg(j.error||"Failed to create â€” may be duplicate"); setCustomerSearching(false); return; }
      ...
      setCustomerMode("found");
      setCustomerMsg(`New customer created: ${j.name} â€¢ ${j.phone} â€” linked to this order`);
  ...
  function continueAsWalkIn(){
    ...
    setCustomerMsg("Walk-in customer â€” no loyalty/coupons/birthday offers (Â§6)");
    ...
    try{
      ...
      if(!r.ok){
        ...
        if(found){
          ...
          if((subtotal+tax-discount) < (found.minSpend||0)){ setCouponMsg(`Min order â‚¹${found.minSpend} required`); return; }
          ...
          setCouponMsg(`Coupon ${found.code} applied: -â‚¹${disc}`);
          return;
        }
      ...
      const j = await r.json();
      if(j.discount) { setCouponDiscount(j.discount); setCouponMsg(`Coupon applied: -â‚¹${j.discount}`); }
    } catch{
  ...
  // hold / resume (Â§7)
  function holdOrder(){
    if(cart.length===0){ setMsg("Cart empty â€” nothing to hold"); return; }
    ...
    setCart([]); setNotes(""); setDiscount(0); setCouponDiscount(0); setMsg(`Order held (#${holdOrders.length+1}) â€” resume from Hold list`);
  }
  ...
  // normalized payments â€” fixes Pay Bill not clickable when paidSum stale (Â§15)
  ...
  async function createOrderAndBill(overridePayments?: PaymentLine[]){
    ...
    if(cart.length===0){ const m="Add at least one item â€” Step 1: select products"; setMsg(m); setLastError(m); window.scrollTo({top:0, behavior:"smooth"}); return; }
    ...
    if(orderType==="DINE_IN" && tableId && tables.length && !tables.some(t=> t.id===tableId)){
      ...
      if(avail){ effectiveTableId = avail.id; setTableId(avail.id); setMsg(`Selected table not found in DB â€” auto-switched to ${avail.number}`); }
      else { const m="Selected table not found â€” please refresh tables and reselect (P2003)"; setMsg(m); setLastError(m); return; }
    }
    if(orderType==="DINE_IN" && !effectiveTableId){
      if(!tablesLoaded){ const m="Tables loading â€” please wait 2 sec and retry"; setMsg(m); setLastError(m); return; }
      const avail = tables.find(t=> t.status==="AVAILABLE" || t.status==="RESERVED");
      if(avail){ effectiveTableId = avail.id; setTableId(avail.id); } else { const m="Step 1: Select table for Dine-in â€” no available table (all OCCUPIED)"; setMsg(m); setLastError(m); window.scrollTo({top:0, behavior:"smooth"}); return; }
    }
    ...
    if(orderType==="DELIVERY" && !resolvedCustomer && resolvedMode!=="walkin" && resolvedMode!=="new"){ const m="Delivery requires customer â€” Search or Add New Customer first (Step 3)"; setMsg(m); setLastError(m); window.scrollTo({top:0, behavior:"smooth"}); return; }
    ...
    if(resolvedMode==="new"){
      if(!newCustomerForm.name.trim() || !/^[6-9]\d{9}$/.test(newCustomerForm.phone)){
        const m="Step 3: New customer â€” Name + 10-digit mobile required"; setMsg(m); setLastError(m); window.scrollTo({top:0, behavior:"smooth"}); return;
      }
      ...
      try{
        ...
        const cj = await cr.json();
        if(!cr.ok){ const m=cj.error||"Customer create failed â€” check phone duplicate"; setMsg(m); setLastError(m); setBusy(false); window.scrollTo({top:0, behavior:"smooth"}); return; }
        ...
        setCustomerMsg(`New customer created: ${cj.name} â€” saved to CRM`);
    ...
    if(Math.abs(paid - grandTotal) > 0.01 && paid < grandTotal){ setMsg(`Payments â‚¹${paid} < total â‚¹${grandTotal} â€” auto-syncing to total`); // auto-fix instead of blocking
    ...
    try {
      ...
      if(earn) setMsg(`âœ… Paid â€¢ Bill ${bJson.billNumber} â€” customer ${customerMode==="found"? customerProfile?.name : effectiveCustomerProfile?.name || customerProfile?.name || "â€”"} â€¢ +${earn} pts (bal ${bal}) â€¢ QR ready â€” saved to CRM`);
      else setMsg(`âœ… Paid â€¢ Bill ${bJson.billNumber} â€” customer ${customerMode==="found"? customerProfile?.name : effectiveCustomerProfile?.name || customerProfile?.name || "â€”"} â€¢ QR ready â€” saved to CRM ${bJson.totalAmount <100 ? "(â‚¹100+ to earn)" : ""}`);
  ...
  async function handleRazorpayPay(){
    ...
    // DINE_IN table will be auto-selected in createOrderAndBill, so don't block here â€” just warn
    if(grandTotal <=0){ setMsg("Cart total is 0 â€” add items"); return; }
    ...
    try{
      ...
      // mock flow â€” no Checkout needed, directly verify
      if(j.mock){
        ...
        setMsg(`Razorpay mock verified â‚¹${grandTotal} â€” creating billâ€¦`);
        await createOrderAndBill([{ method:"ONLINE", amount: grandTotal, reference: mockPaymentId }]);
        setRazorPayBusy(false);
        return;
      }
      const loaded = await loadRazorpayScript();
      if(!loaded){ setMsg("Razorpay SDK failed to load â€” check internet"); setRazorPayBusy(false); return; }
      const options = {
        key: j.keyId,
        amount: j.amount,
        currency: j.currency || "INR",
        name: "Spice Garden",
        description: `POS bill â‚¹${grandTotal} â€¢ ${cart.length} items`,
        order_id: j.orderId,
        handler: async (resp: { razorpay_order_id:string; razorpay_payment_id:string; razorpay_signature:string })=>{
          try{
            ...
            if(!vr.ok){ setMsg(vj.error||"Razorpay verify failed â€” payment not captured"); setRazorPayBusy(false); return; }
            setMsg(`Razorpay success ${resp.razorpay_payment_id} â€” creating billâ€¦`);
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <Badge className="border bg-white dark:bg-zinc-900 dark:text-zinc-100 dark:border-zinc-700 px-2.5 py-1 text-xs font-medium">{orderType} â€¢ {cart.length} lines â€¢ â‚¹{grandTotal.toLocaleString("en-IN")}</Badge>
        ...
        {busy && <div className="text-xs text-zinc-500 dark:text-zinc-400 flex items-center gap-2"><span className="h-3 w-3 border-2 border-zinc-300 border-t-zinc-900 dark:border-zinc-600 dark:border-t-white rounded-full animate-spin" /> Processing â€” please waitâ€¦</div>}
      </div>
          ...
          <Card className="shadow-sm border-zinc-200 dark:border-zinc-800">
            <CardHeader className="pb-3 border-b dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50">
              <div className="flex flex-wrap gap-3 items-center justify-between">
                <CardTitle className="text-[15px] font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">Menu â€” tap to add <span className="font-normal text-zinc-500 text-xs ml-1">({filteredMenu.length})</span></CardTitle>
              ...
              <CardDescription className="flex flex-wrap gap-2 items-center pt-2">
                <select value={tableId} onChange={e=>setTableId(e.target.value)} className="border rounded-lg px-3 py-2 text-sm bg-white dark:bg-zinc-800 dark:text-zinc-100 dark:border-zinc-700 focus:outline-none focus:ring-1 focus:ring-zinc-900 dark:focus:ring-zinc-100" disabled={orderType!=="DINE_IN"}>
                  ...
                  {tables.slice(0,40).map(t=><option key={t.id} value={t.id}>{t.number} â€” {t.status} â€¢ {t.capacity}pax</option>)}
                </select>
            ...
            <CardContent className="p-3">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 max-h-[68vh] overflow-auto pr-1">
                {filteredMenu.map(m=>(
                  <div key={m.id} className={`group border rounded-xl p-3.5 space-y-2.5 bg-white dark:bg-zinc-900 dark:border-zinc-800 shadow-sm hover:shadow-md transition-all ${m.isAvailable?"hover:border-zinc-300 dark:hover:border-zinc-700":"opacity-60"}`}>
                    ...
                    <div className="flex items-baseline gap-1">
                      <span className="text-base font-bold tracking-tight text-zinc-900 dark:text-zinc-100">â‚¹{m.price}</span>
                    ...
                    {Boolean(m.variants?.length) && <select defaultValue="" onChange={e=>{ if(e.target.value) addToCart(m, e.target.value); e.target.value=""; }} className="w-full border rounded-lg h-8 text-xs px-2 bg-white dark:bg-zinc-800 dark:text-zinc-100 dark:border-zinc-700"><option value="">Variantâ€¦</option>{m.variants!.map(v=><option key={v.id} value={v.id}>{v.name} (+â‚¹{v.priceDelta})</option>)}</select>}
                    {Boolean(m.addOns?.length) && <div className="text-xs space-y-1">{m.addOns!.slice(0,2).map(a=> <label key={a.id} className="flex items-center gap-1.5 text-zinc-700 dark:text-zinc-300"><input type="checkbox" className="rounded border-zinc-300 dark:border-zinc-600" onChange={e=>{ const checked=e.target.checked; if(checked) addToCart(m, undefined,[a.id]); }} /> {a.name} <span className="text-zinc-500">(+â‚¹{a.price})</span></label>)}</div>}
                ...
                {filteredMenu.length===0 && <div className="col-span-3 text-center text-sm text-zinc-500 dark:text-zinc-400 py-10 border border-dashed dark:border-zinc-700 rounded-xl bg-zinc-50 dark:bg-zinc-900">No items in this category â€” try another filter</div>}
              </div>
            </CardContent>
          </Card>
          {holdOrders.length>0 && (
            <Card className="border-amber-200 dark:border-amber-800 bg-amber-50/50 dark:bg-amber-950/20">
              <CardHeader className="pb-2"><CardTitle className="text-sm font-semibold text-amber-900 dark:text-amber-100">Held Orders â€” Resume</CardTitle></CardHeader>
              <CardContent className="space-y-2">
                {holdOrders.map(h=>(
                  <div key={h.id} className="flex items-center justify-between border rounded-lg p-3 text-sm bg-white dark:bg-zinc-900 dark:border-zinc-700">
                    <span className="text-zinc-700 dark:text-zinc-300">{h.orderType} â€¢ {h.cart.reduce((a,c)=>a+c.quantity,0)} items â€¢ {new Date(h.createdAt).toLocaleTimeString()} â€¢ Table {h.tableId? tables.find(t=>t.id===h.tableId)?.number||h.tableId.slice(0,6):"â€”"}</span>
          ...
          <Card className="shadow-sm border-zinc-200 dark:border-zinc-800">
            <CardHeader className="pb-3 border-b dark:border-zinc-800 bg-white dark:bg-zinc-900 sticky top-0 z-10">
              <CardTitle className="text-[15px] font-semibold flex items-center gap-2 text-zinc-900 dark:text-zinc-100">
                ...
                {cart.length>0 && <Badge className="ml-auto bg-zinc-900 text-white dark:bg-white dark:text-zinc-900 border-0">â‚¹{grandTotal.toLocaleString("en-IN")}</Badge>}
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4 p-4">
              {cart.length===0? <div className="text-sm text-zinc-500 dark:text-zinc-400 border-2 border-dashed dark:border-zinc-700 rounded-xl p-8 text-center bg-zinc-50 dark:bg-zinc-900/50">ðŸ›’ No items â€” tap menu to add<br/><span className="text-xs">Your cart is empty</span></div> :
                  ...
                  {cart.map(c=>(
                    <div key={c.key} className="flex gap-3 items-start border rounded-xl p-3 bg-white dark:bg-zinc-900 dark:border-zinc-700 shadow-sm">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-zinc-500 dark:text-zinc-400 font-medium">â‚¹{c.unitPrice} Ã— {c.quantity} = <span className="text-zinc-900 dark:text-zinc-100 font-bold">â‚¹{c.unitPrice*c.quantity}</span></div>
              ...
              <Textarea placeholder="Order notes (optional) â€” e.g. less spicy" value={notes} onChange={e=>setNotes(e.target.value)} rows={2} className="bg-white dark:bg-zinc-800" />
              ...
              <div className="border rounded-xl p-4 space-y-3 bg-zinc-50 dark:bg-zinc-900 dark:border-zinc-700">
                ...
                {customerMode==="found" && customerProfile && (
                  <div className="space-y-2.5 text-sm bg-white dark:bg-zinc-800 border dark:border-zinc-700 rounded-xl p-3.5 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      ...
                      <span className="bg-green-50 dark:bg-green-950/30 rounded-lg p-2 border border-green-200 dark:border-green-800">Spend: <b className="text-green-700 dark:text-green-300">â‚¹{Math.round(customerProfile.totalSpending ?? customerProfile.totalSpend ?? 0)}</b></span>
                      ...
                      <span className="bg-zinc-50 dark:bg-zinc-900 rounded-lg p-2 border dark:border-zinc-700">Last: <b>{customerProfile.lastVisitDate? new Date(customerProfile.lastVisitDate).toLocaleDateString():"â€”"}</b></span>
                    </div>
                    {Boolean(customerProfile.availableCoupons?.length) && (
                      <div className="text-xs">
                        ...
                        {customerProfile.availableCoupons!.slice(0,3).map(c=> <div key={c.code} className="font-mono border rounded-lg px-2.5 py-1.5 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-800 mt-1.5 text-amber-900 dark:text-amber-100">{c.code} â€¢ {c.value}{c.rewardType==="PERCENTAGE"?"%":"â‚¹"} off â€¢ exp {new Date(c.expiryDate).toLocaleDateString()}</div>)}
                      </div>
                    )}
                    <div className="text-xs font-medium text-green-700 dark:text-green-300 bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 rounded-lg px-2.5 py-1.5">âœ“ Linked â€” Loyalty & coupons will apply</div>
                  </div>
                )}
                {customerMode==="new" && (
                  <div className="space-y-2.5 bg-white dark:bg-zinc-800 border dark:border-zinc-700 rounded-xl p-3.5 shadow-sm">
                    <div className="text-sm font-bold text-zinc-900 dark:text-zinc-100">New Customer â€” fill & create</div>
                ...
                {customerMode==="walkin" && (
                  <div className="text-sm bg-white dark:bg-zinc-800 border dark:border-zinc-700 rounded-xl p-3.5">
                    ...
                    <Button size="sm" variant="outline" className="mt-3 h-8" onClick={()=> setCustomerMode("none")}>Change â€” search customer</Button>
                  </div>
                )}
              ...
              <div className="border rounded-xl p-4 space-y-2.5 text-sm bg-white dark:bg-zinc-900 dark:border-zinc-700 shadow-sm">
                ...
                <div className="space-y-1.5 text-sm">
                  <div className="flex justify-between text-zinc-600 dark:text-zinc-400"><span>Subtotal</span><span className="font-medium text-zinc-900 dark:text-zinc-100">â‚¹{subtotal.toLocaleString("en-IN")}</span></div>
                  <div className="flex justify-between text-zinc-600 dark:text-zinc-400"><span>Tax 5% (server validated)</span><span className="font-medium">â‚¹{tax}</span></div>
                  ...
                  <div className="space-y-1.5 pt-2 border-t dark:border-zinc-700">
                    ...
                    {couponDiscount>0 && <div className="flex justify-between text-green-600 dark:text-green-400 font-semibold bg-green-50 dark:bg-green-950/30 rounded-lg px-2.5 py-1.5"><span>Coupon Discount</span><span>-â‚¹{couponDiscount}</span></div>}
                  </div>
                  {customerMode==="found" && (customerProfile?.loyaltyPoints||0) > 0 && (
                    <div className="flex gap-2 items-center bg-orange-50 dark:bg-orange-950/20 border border-orange-200 dark:border-orange-800 rounded-lg p-2.5">
                      <Input type="number" placeholder={`Redeem (avail ${customerProfile?.loyaltyPoints})`} value={loyaltyRedeem||""} onChange={e=>{
                        ...
                        if(v>avail){ setMsg(`Cannot redeem ${v} â€” only ${avail} available`); return; }
                        setLoyaltyRedeem(v);
                      }} className="h-8 flex-1 bg-white dark:bg-zinc-800" />
                      <span className="text-xs font-medium text-orange-700 dark:text-orange-300 whitespace-nowrap">1 pt = â‚¹1</span>
                    </div>
                  )}
                  {loyaltyRedeem>0 && <div className="flex justify-between text-green-600 dark:text-green-400 font-semibold bg-green-50 dark:bg-green-950/30 rounded-lg px-2.5 py-1.5"><span>Loyalty Redeem</span><span>-â‚¹{loyaltyRedeem}</span></div>}
                  <div className="flex justify-between font-bold text-[16px] border-t dark:border-zinc-700 pt-3 text-zinc-900 dark:text-zinc-100"><span>Grand Total</span><span>â‚¹{grandTotal.toLocaleString("en-IN")}</span></div>
                  <div className="text-xs text-zinc-500 dark:text-zinc-400">All totals recomputed server-side â€” client not trusted.</div>
                </div>
              </div>

              {/* Payments */}
              <div className="border rounded-xl p-4 space-y-3 bg-zinc-50 dark:bg-zinc-800/50 dark:border-zinc-700">
                <div className="flex items-center justify-between"><span className="text-sm font-bold flex items-center gap-2 text-zinc-900 dark:text-zinc-100"><span className="h-5 w-5 rounded-full bg-zinc-900 text-white dark:bg-white dark:text-zinc-900 flex items-center justify-center text-[10px] font-bold">â‚¹</span>Payments â€” Split allowed</span><Button size="sm" variant="outline" onClick={addPayment} className="h-7 text-xs font-semibold rounded-full">+ Split</Button></div>
                {payments.map((p,idx)=>(
                ...
                <div className="flex justify-between text-xs font-medium"><span className="text-zinc-600 dark:text-zinc-400">Paid â‚¹{paidSum.toLocaleString("en-IN")}</span><span className={remaining===0?"text-green-600 dark:text-green-400 font-bold":"text-amber-600 dark:text-amber-400"}>{remaining===0?"âœ“ Fully paid":`Remaining â‚¹${remaining.toLocaleString("en-IN")}`}</span></div>
              </div>
              ...
              {order && bill && (
                <div id="bill-success" className="rounded-2xl bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/30 dark:to-emerald-950/20 border-2 border-green-400 dark:border-green-700 p-4 space-y-3 shadow-sm">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <div className="font-bold text-green-800 dark:text-green-200 text-[15px] leading-tight">Payment Successful â€” Bill {bill.billNumber}</div>
                      <div className="text-xs text-green-700 dark:text-green-300 mt-1">Order {order.orderNumber} â†’ Customer: {customerMode==="found"? customerProfile?.name : customerMode==="walkin"?"Walk-in":"â€”"} â€¢ Total â‚¹{bill.totalAmount.toLocaleString("en-IN")}</div>
                    </div>
                  ...
                  <div className="flex gap-3 items-center bg-white dark:bg-zinc-900 rounded-xl p-3 border dark:border-zinc-700 shadow-sm">
                    <img src={`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(typeof window!=="undefined"? `${window.location.origin}/qr/${bill.qrToken}?billId=${bill.id}` : `/qr/${bill.qrToken}`)}`} alt="Loyalty QR" className="h-24 w-24 border rounded-xl bg-white p-1" />
                      ...
                      <div className="font-bold text-zinc-900 dark:text-zinc-100">Loyalty QR â€” Scan for reward</div>
                      ...
                      <div className="text-zinc-600 dark:text-zinc-400 font-medium text-xs">
                        {(bill as unknown as {loyaltyEarned?:number}).loyaltyEarned
                          ...
                          : bill.totalAmount>=100? `Earn ~${Math.floor(bill.totalAmount/100)*10} pts (â‚¹100=10) â€” next bill` : "Spend â‚¹100+ to earn loyalty"}
                      </div>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" variant="outline" className="flex-1 h-9 font-semibold rounded-xl" onClick={()=>{ setCart([]); setOrder(null); setBill(null); setMsg("New bill started â€” add products"); window.scrollTo({top:0,behavior:"smooth"}); }}>+ New Bill</Button>
          ...
          {bill && (
            <Card id="receipt" className="shadow-lg border-zinc-300 dark:border-zinc-700 print:shadow-none overflow-hidden">
              <CardHeader className="bg-zinc-900 text-white dark:bg-zinc-800 border-b-0">
                ...
                <CardDescription className="text-zinc-300 dark:text-zinc-400">Order #{order?.orderNumber || bill.orderId.slice(0,8)} â€¢ {new Date(bill.paidAt || Date.now()).toLocaleString()} â€¢ Table {tables.find(t=>t.id===tableId)?.number || "â€”"}</CardDescription>
              </CardHeader>
              <CardContent className="p-5 space-y-4 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100">
                ...
                <div className="space-y-1.5 text-xs">
                  ...
                  <div className="flex justify-between"><span className="text-zinc-500 dark:text-zinc-400">Paid at</span><span className="font-medium">{bill.paidAt? new Date(bill.paidAt).toLocaleString():"â€”"}</span></div>
                </div>
                <div className="border-t-2 border-dashed border-zinc-200 dark:border-zinc-700 my-1" />
                <div className="space-y-1.5 text-xs">
                  <div className="flex justify-between"><span className="text-zinc-600 dark:text-zinc-400">Subtotal</span><span className="font-medium">â‚¹{bill.subtotal.toLocaleString("en-IN")}</span></div>
                  <div className="flex justify-between"><span className="text-zinc-600 dark:text-zinc-400">Tax (5%)</span><span className="font-medium">â‚¹{bill.taxAmount.toLocaleString("en-IN")}</span></div>
                  <div className="flex justify-between"><span className="text-zinc-600 dark:text-zinc-400">Discount</span><span className="font-medium">-â‚¹{bill.discountAmount.toLocaleString("en-IN")}</span></div>
                  {(bill as unknown as {couponDiscount:number}).couponDiscount ? <div className="flex justify-between text-green-600 dark:text-green-400 font-semibold"><span>Coupon</span><span>-â‚¹{(bill as unknown as {couponDiscount:number}).couponDiscount}</span></div> : null}
                  ...
                  <div className="flex justify-between font-black text-[16px] border-t-2 border-zinc-900 dark:border-zinc-100 pt-3"><span>Total Paid</span><span>â‚¹{bill.totalAmount.toLocaleString("en-IN")}</span></div>
                </div>
                <div className="border-2 border-dashed rounded-xl p-3 bg-zinc-50 dark:bg-zinc-800 dark:border-zinc-600 flex gap-3 items-center">
                  ...
                  <div className="text-xs space-y-1">
                    ...
                    <div className="text-zinc-600 dark:text-zinc-400">Scan to review â€” 4â˜…=40% off, 5â˜…=50% off</div>
